...
if(myfolder_vc %in% dir() == FALSE){
	...
	system(paste("gcta64 --bfile Data_clean --thread-num ",num_nodes," --autosome-num 39 --make-grm --out ",myfolder_vc,"/TEMP",sep=""))
	system(paste("gcta64 --bfile Data_clean --thread-num ",num_nodes," --autosome-num 39 --make-grm-d --out ",myfolder_vc,"/TEMP",sep=""))
}
...
try({
	...
	if(!file.exists(paste(outdir,'/2_Pvalue/Pvalue_',phenotype_name,'.txt.tar.gz',sep=''))){
		...
		if(Run_separated == F){			
			...
			if(!file.exists("Data_clean_recode12.ped")){system("plink --noweb --silent --chr-set 39 --bfile Data_clean --recode12 --out Data_clean_recode12")}
			...
			if(myfolder_separated %in% dir() == FALSE){
				...
				for(i in 1:length(Part_num)){
					system(paste("plink --noweb --silent --chr-set 39 --bfile Data_clean --chr ",Part_num[i]," --recode12 --out ",myfolder_separated,"/Part",i,sep=""))						
