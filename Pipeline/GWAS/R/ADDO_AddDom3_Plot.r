...
try({
	...
	try({
		...
		thrgenome = -log10(0.05/1000000); thrsuggest = -log10(1/1000000)
	...
	try({
		...
		try({
			...
			system(paste("plink --noweb --silent --chr-set 39 --bfile Data_clean --r2 --ld-snp ",peakSNP,"--ld-window-kb ",RegionMan_chr_region," --ld-window 99999 --ld-window-r2 0","--out RegionMan_tmp"))
	...
	try({
		...
		try({
			...
			setwd(rawdir); system(paste("plink --noweb --silent --chr-set 39 --bfile Data_clean --snp",snp_name,"--recode --out Boxplot_tmp"))
