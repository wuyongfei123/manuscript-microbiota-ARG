...
if(Input_type == "PLINK"){
	...
	if(!file.exists("Data_clean.bed")){system(paste("plink --noweb --silent --bfile ",indir,"/",Input_name," --maf ",GT_maf," --geno ",GT_missing," --chr-set 39 --make-bed --out Data_clean",sep=""))}
	if(!file.exists("Data_clean.frq")){system("plink --noweb --silent --bfile Data_clean --chr-set 39 --freq --out Data_clean")}
...
if(Input_type == "GenABEL"){
	...
	if(!file.exists("Data_raw.tped")){system("plink --noweb --silent --file Data_raw --transpose --chr-set 39 --recode --out Data_raw")}
	if(!file.exists("Data_clean.bed")){system(paste("plink --noweb --silent --tfile Data_raw --maf ",GT_maf," --geno ",GT_missing," --chr-set 39 --make-bed --out Data_clean",sep=""))}
	if(!file.exists("Data_clean.frq")){system("plink --noweb --silent --bfile Data_clean --freq --chr-set 39 --out Data_clean")}
	Clean_map = read("Data_clean.bim")
...
try({
	if(dim(Clean_map)[1] <= 100000){
		...
		system("plink --noweb --silent --bfile Data_clean --extract LD_tmp.txt --chr-set 39 --recode --out LD_tmp")
		system("plink --noweb --silent --bfile Data_clean --r2 --ld-window 99999 --ld-window-r2 0 --chr-set 39 --out LD_tmp")
...
try({
	if(dim(Clean_map)[1] <= 100000){
		setwd(paste(outdir,"/1_PheGen/0_Data",sep=""))
		system("plink --noweb --silent --bfile Data_clean --cluster --matrix --chr-set 39 --out IBS_tmp")
...
if(Kinship_type == "GenABEL"){
	...
	if(!file.exists("Data_clean.tped")){system("plink --noweb --silent --bfile Data_clean --transpose --chr-set 39 --recode --out Data_clean")}
	...
	system("plink --noweb --silent --bfile Data_clean --indep-pairwise 50 10 0.5 --chr-set 39 --out tmp_genabel")
...
if(Kinship_type == "EMMA"){
	if(!file.exists("tmp_emma.raw")){system("plink --noweb --silent --bfile Data_clean --recodeA --chr-set 39 --out tmp_emma")}
...
if(Kinship_type == "EMMAX"){
	if(!file.exists("tmp_emmax.tped")){system("plink --noweb --silent --bfile Data_clean --recode12 --output-missing-genotype 0 --transpose --chr-set 39 --out tmp_emmax")}
...
if(Kinship_type == "GCTA"){
	system(paste("gcta64 --bfile Data_clean --thread-num ",num_nodes," --autosome-num 39 --make-grm-gz --out tmp_gcta_add",sep=""))	
...
if(Kinship_type == "GCTA_ad"){
	...
	system(paste("gcta64 --bfile Data_clean --thread-num ",num_nodes," --autosome-num 39 --make-grm-d-gz --out tmp_gcta_dom",sep=""))
...
if(Kinship_type == "HOMEBREW_AFW"){
	...
	system("plink --noweb --silent --bfile Data_clean --recode --chr-set 39 --out tmp_matrix")
