...
gut_ARG_data <- read_csv("tetracycline_gut_abundance.csv")

# 进行Wilcoxon检验并计算p值
comparisons <- combn(unique(gut_ARG_data$gut_locations), 2, simplify = FALSE)
p_values <- sapply(comparisons, function(x) {
  wilcox.test(tetracycline_abundance ~ gut_locations, data = gut_ARG_data%>% filter(gut_locations %in% x))$p.value
})
...
#绘图
p <- ggplot(gut_ARG_data, aes(x = gut_locations, y = tetracycline_abundance)) +
  ...
  scale_y_continuous(
    limits = c(0, max(gut_ARG_data$tetracycline_abundance) * 1.5), 
    expand = c(0, 0)
  ) +
  ...
  geom_text(
    ...
    aes(
      x = gut_locations, 
      y = max(gut_ARG_data$tetracycline_abundance) * 1.3, 
      label = letter
    ), 
    size = 6
  ) +
