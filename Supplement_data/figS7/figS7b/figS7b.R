...
df = read.delim('s_P.coprocola.GWAS.region.tsv',header = T)
df$r2 = df$R2
...
P = ggplot(df, aes(x = POS / 1e6, y = P_Dom, color = r2)) +
  ...
  #inherit.aes = FALSE) +  # 阻止继承主图层的aes
  coord_cartesian(ylim = c(-3, max(df$P_Dom) * 1.1)) +  #扩大y轴0以下的空间
...
ggsave(P, filename="s_P.coprocola.pdf",width = 10,height = 8) 